...
test_state_combination() {
    ...
    if [ "$has_staged_del" = "1" ] || [ "$has_unstaged_del" = "1" ]; then
        ...
        git add deleteme.txt >/dev/null 2>&1
        git commit -m "add deleteme" >/dev/null 2>&1
    fi

    if [ "$has_both_mod" = "1" ]; then
        echo "both-modified" > both.txt
        git add both.txt >/dev/null 2>&1
        git commit -m "add both.txt" >/dev/null 2>&1
    fi

    if [ "$has_staged_mod" = "1" ]; then
        echo "staged-mod" > staged_mod.txt
        git add staged_mod.txt >/dev/null 2>&1
        git commit -m "add staged_mod" >/dev/null 2>&1
    fi

    if [ "$has_unstaged_mod" = "1" ]; then
        echo "unstaged-mod" > unstaged_mod.txt
        git add unstaged_mod.txt >/dev/null 2>&1
        git commit -m "add unstaged_mod" >/dev/null 2>&1
    fi

    # Create the test state
    if [ "$has_staged_new" = "1" ]; then
        echo "new staged content" > staged_new.txt
        git add staged_new.txt >/dev/null 2>&1
    fi
    ...
    if [ "$has_staged_del" = "1" ]; then
        git rm deleteme.txt >/dev/null 2>&1
    fi
    ...
    local combo_desc="staged_new=$has_staged_new untracked=$has_untracked unstaged_mod=$has_unstaged_mod staged_mod=$has_staged_mod both_mod=$has_both_mod staged_del=$has_staged_del unstaged_del=$has_unstaged_del"
